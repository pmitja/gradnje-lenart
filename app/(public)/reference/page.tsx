...
export const metadata: Metadata = {
  ...
  description:
    'Odkrijte naše zaključene projekte in nepremičnine, ki smo jih uspešno zgradili za zadovoljne stranke.',
  openGraph: {
    title: 'Zaključeni projekti | Gradnje Plus',
  ...
  alternates: {
    canonical: 'https://gradnje-lenart.si/reference',
  },
}
...
export default async function ReferencePage() {
  ...
  // Create JSON-LD data as a string
  const jsonLdString = `{
        ...
        ${inactiveLocations
          .map((location, index) => {
            ...
            "item": {
              ...
              "description": "${(location.description || `Zaključen nepremičninski projekt v ${location.city}`).replace(/"/g, '\\"')}",
              "url": "https://gradnje-lenart.si/reference#${location.id}",
              "address": {
                "@type": "PostalAddress",
                "addressLocality": "${location.city.replace(/"/g, '\\"')}",
                "streetAddress": "${location.address.replace(/"/g, '\\"')}",
                "addressCountry": "SI"
              }${
                displayImage
                  ? `,
              "image": "${displayImage}"`
                ...
                location.createdAt
                  ? `,
              "dateCreated": "${new Date(location.createdAt).toISOString()}"`
                  : ''
              },
    ...
    <>
      ...
      <div className="min-h-screen">
        ...
        <div className="relative w-full">
          <div className="absolute inset-x-0 left-1/2 right-1/2 ml-[-50vw] mr-[-50vw] h-full w-screen bg-hero-page bg-cover">
          ...
          <div className="relative z-10 py-24 pt-32 lg:py-28 lg:pt-36 xl:py-32 xl:pt-40">
            <div className="mx-auto max-w-3xl text-center">
              ...
              <p className="mx-auto mb-8 max-w-2xl text-lg text-secondary-400 md:text-xl">
                Odkrijte kakovost in inovativnost naših zaključenih nepremičninskih projektov, ki
                pričajo o naši zavezanosti odličnosti.
              </p>
        ...
        <div>
          ...
          <section className="py-16 sm:px-6 lg:py-24">
            ...
            {inactiveLocations.length === 0 ? (
              <div className="mx-auto flex max-w-2xl flex-col items-center rounded-xl bg-white p-8 text-center shadow-md">
                ...
                <p className="mb-6 text-secondary-400">
                  Trenutno nimamo zaključenih projektov za prikaz. Preverite naše aktivne projekte,
                  ki so v teku.
                </p>
                <Link
                  href="/projekti"
                  className="rounded-md bg-primary-400 px-6 py-3 font-semibold text-white transition hover:bg-primary-500"
                >
                      ...
                      <div className="ml-16 w-full md:ml-0 md:w-1/2 md:px-8">
                        <div className="rounded-lg bg-white p-4 shadow-md sm:p-6">
                          ...
                          <div className="mt-3 flex items-center text-xs text-secondary-400 sm:text-sm">
                            ...
                            <span>
                              {location.realEstates.length}{' '}
                              {location.realEstates.length === 1 ? 'nepremičnina' : 'nepremičnin'}
                            </span>
                          </div>
                        </div>
                      </div>
